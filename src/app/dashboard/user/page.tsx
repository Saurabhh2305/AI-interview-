...
export default function UserDashboard() {
  ...
  const loadUserData = () => {
    try {
      ...
      const photo =
        localStorage.getItem(`userPhoto_${userId}`) || parsed.photo || null;
      const resume =
        localStorage.getItem(`userResume_${userId}`) || parsed.resume || null;
  ...
  // ✅ Load data initially and when storage changes
  useEffect(() => {
    loadUserData();
  ...
  const userResume = user?.resume || null;
  const isProfileComplete = userPhoto && userResume;
